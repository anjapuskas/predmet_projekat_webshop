...
export const VerifyUser = async (id) => {
    try {
        const response = await axiosInstance.put(`${process.env.REACT_APP_BACKEND_APPLICATION_ENDPOINT}/users/verify/${id}`);
        return response.data;
    } catch (error) {
     throw new Error(error.response.data.error);
    }
...
export const RejectUser = async (id) => {
    try {
        const response = await axiosInstance.put(`${process.env.REACT_APP_BACKEND_APPLICATION_ENDPOINT}/users/reject/${id}`);
        return response.data;
    } catch (error) {
     throw new Error(error.response.data.error);
    }
